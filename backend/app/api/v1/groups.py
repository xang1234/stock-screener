...
router = APIRouter()
SUPPORTED_GROUP_MARKETS = {"US", "HK", "IN", "JP", "KR", "TW"}
...
def _normalize_market_param(market: str | None) -> str:
    ...
    if normalized not in SUPPORTED_GROUP_MARKETS:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported market '{market}'. Expected one of: US, HK, IN, JP, KR, TW.",
        )
    return normalized
...
async def get_current_rankings(
    ...
    market: str = Query("US", description="Market code: US, HK, IN, JP, KR, or TW"),
