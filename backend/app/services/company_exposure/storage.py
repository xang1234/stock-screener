...
Garbage collection (scheduled daily) removes only unreferenced blobs older
than 30 days and abandoned temp files older than 24 hours, rechecking
references under an exclusive storage lock that acquisition also takes
(shared) when it binds a revision to an existing blob. Every removal leaves a
tombstone. Published history is never evicted to make room.
"""
