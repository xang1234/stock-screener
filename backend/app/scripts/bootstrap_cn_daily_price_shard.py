...
def select_shard_symbols(
    ...
    *,
    ...
    ordered = sorted({str(symbol or "").strip().upper() for symbol in symbols if str(symbol or "").strip()})
    return [
...
def bootstrap_cn_daily_price_shard(
    *,
    ...
    for batch_number, batch_symbols in enumerate(_iter_chunks(missing_symbols, batch_size), start=1):
        print(
    ...
    export_stats = service.export_daily_price_bundle(
        ...
        latest_manifest_path=output_dir / manifest_name,
        as_of_date=as_of_date,
        symbols=shard_symbols,
    )
...
def main() -> int:
    ...
    parser.add_argument("--shard-index", type=int, required=True, help="1-based shard index to refresh.")
