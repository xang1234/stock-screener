...
from ..infra.db.portability import column_names, is_sqlite, table_names
from ..models.provider_snapshot import ProviderSnapshotPointer, ProviderSnapshotRow, ProviderSnapshotRun
...
def migrate_provider_snapshot_tables(engine) -> None:
    ...
    with engine.connect() as conn:
        if is_sqlite(conn):
            ...
            ProviderSnapshotRun.__table__.create(bind=conn, checkfirst=True)
            ProviderSnapshotRow.__table__.create(bind=conn, checkfirst=True)
            ProviderSnapshotPointer.__table__.create(bind=conn, checkfirst=True)
        conn.execute(
            text(
                """
