...
from .portability import column_names, index_defs, table_names
...
IndexRequirement = tuple[tuple[str, ...], bool]


def _index(*columns: str, unique: bool = False) -> IndexRequirement:
    return (tuple(columns), unique)
...
_THEME_MERGE_SAFETY_REQUIRED_INDEXES = {
    _index("pair_min_cluster_id", "pair_max_cluster_id", unique=True),
    _index("approval_idempotency_key"),
    _index("pair_min_cluster_id"),
    _index("pair_max_cluster_id"),
}
...
_UNIVERSE_LIFECYCLE_REQUIRED_INDEXES = {
    _index("exchange", "status"),
    _index("status", "is_active"),
}
_UNIVERSE_STATUS_EVENT_INDEXES = {
    _index("symbol", "created_at"),
    _index("new_status", "created_at"),
}
...
def _verify_columns_and_indexes(
    ...
    *,
    ...
    required_indexes: set[IndexRequirement],
) -> dict[str, Any]:
    with engine.connect() as conn:
        tables = table_names(conn)
        table_exists = table_name in tables
        columns = column_names(conn, table_name) if table_exists else set()
        indexes = index_defs(conn, table_name) if table_exists else []

    missing_columns = sorted(required_columns - columns)
    missing_indexes = [
        _format_index_requirement(columns, unique)
        for columns, unique in sorted(required_indexes)
        if not _index_requirement_satisfied(indexes, columns=columns, unique=unique)
    ]
    return {
...
def _index_requirement_satisfied(
    ...
    *,
    ...
    for index in existing_indexes:
        existing_columns = tuple(index.get("columns") or [])
        existing_unique = bool(index.get("unique"))
...
def _format_index_requirement(columns: tuple[str, ...], unique: bool) -> str:
    prefix = "UNIQUE" if unique else "INDEX"
