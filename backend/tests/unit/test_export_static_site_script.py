...
def test_main_restores_and_reexports_compact_rrg_history(monkeypatch, tmp_path):
    calls = []
    input_path = tmp_path / "rrg-history-hk.json.gz"
    output_path = tmp_path / "updated-rrg-history-hk.json.gz"
    ...
    class _HistoryService:
        def import_bundle(self, db, *, market, input_path):
            calls.append(("import", db, market, input_path))
            return {"rows": 12}

        def export_bundle(self, db, *, market, output_path, through_date):
            calls.append(("history-export", db, market, output_path, through_date))
            return {"rows": 13}
    ...
    class _ExportService:
        ...
        def export(self, output_dir, **_kwargs):
            calls.append(("site-export", output_dir))
            return SimpleNamespace(
                output_dir=output_dir,
    ...
    monkeypatch.setattr(export_script, "SessionLocal", _Session)
    ...
    assert export_script.main() == 0
    assert [call[0] for call in calls] == ["import", "site-export", "history-export"]
    assert calls[0][2:] == ("HK", input_path)
    assert calls[2][2:] == ("HK", output_path, date(2026, 7, 10))


def test_main_bootstraps_when_rolling_rrg_history_is_invalid(monkeypatch, tmp_path, capsys):
    class _Session:
        def __enter__(self):
            return self
    ...
    class _HistoryService:
        def import_bundle(self, *_args, **_kwargs):
            raise export_script.StaticRRGHistoryBundleError("wrong market")

        def export_bundle(self, *_args, **_kwargs):
            return {"rows": 12}
    ...
    monkeypatch.setattr(
        ...
        [
            ...
            "--rrg-history-output",
            str(tmp_path / "updated.json.gz"),
        ],
    )

    assert export_script.main() == 0
    assert "invalid and will be bootstrapped: wrong market" in capsys.readouterr().out
