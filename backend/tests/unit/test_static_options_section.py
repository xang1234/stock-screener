...
def test_combined_section_advertises_selected_options_in_both_manifests(
    ...
    assert selector.calls[0]["equity_generated_at"] == "2026-09-03T22:00:00Z"
...
def test_disabled_combined_section_removes_inherited_options_advertisements(
    tmp_path: Path,
) -> None:
    us_entry = {
        **_entry(),
    ...
    result = StaticOptionsSection(enabled=False).compose_combined(
        output_dir=tmp_path,
        manifest=manifest,
