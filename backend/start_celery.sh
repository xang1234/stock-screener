...
ENABLED_MARKETS="${ENABLED_MARKETS:-US,HK,IN,JP,KR,TW}"
...
./venv/bin/celery -A app.celery_app worker \
    ...
    --pool="$POOL" \
    --concurrency=1 \
    -Q data_fetch_shared,data_fetch_us,data_fetch_hk,data_fetch_in,data_fetch_jp,data_fetch_kr,data_fetch_tw \
