...
- **Deferred until a caller exists.** An over-engineering pass removed code that S1 never calls: the Social attestation import (`import-identities`), storage garbage collection and authorized original reads, the blocking-only safety check for automatic use, job pause/cancel, scheduled period close, the evidence-DAG check, the paid-search/dispatch predicates, and the non-overlapping sum. Each returns with the task that first calls it (Tasks 13, 18–20, 23–24, 27B). Cross-listing tests now build links through the reviewed `propose_link`/`apply_link` path, and I09 is covered by generated or classifier output never counting as primary support.

## S1 delivery record (US verify-only shadow)

- Tasks: 00–07, 09, 14–16, 17A, 21A, 22A, 26A, 27A on branch `claude/brave-carson-9i90ws`.
- Backend S1 gate: `scripts/run_required_company_exposure_postgres.py --slice S1` on PostgreSQL 16 (local disposable cluster) — 274 passed, all required case/layer pairs and named invariants passed.
